...
def calculate_draft(DX, corr_sound_speed_CTD = True, qual_thr = 8000,
    ...
    if LE_AST_max_sep:
        condition = (
            np.abs(DX.SURFACE_DEPTH_LE - DX.SURFACE_DEPTH_AST)<LE_AST_max_sep
            )

        _N_LE_before = (~np.isnan(DX['SURFACE_DEPTH_LE'])).sum().item()
        DX['SURFACE_DEPTH_LE'] = DX['SURFACE_DEPTH_LE'].where(condition)
        _N_LE_after = (~np.isnan(DX['SURFACE_DEPTH_LE'])).sum().item()
        _pct_reduction = 100*(_N_LE_before - _N_LE_after)/_N_LE_before

        DX['SURFACE_DEPTH_LE'].attrs['note'] += (
...
def calculate_surface_position(DX, corr_sound_speed_CTD = True,
    ...
    if hasattr(DX, 'sound_speed_CTD') and corr_sound_speed_CTD:
        ...
        sound_speed_ratio_obs_nom = (
            DX.sound_speed_CTD.data[:, np.newaxis]
    ...
    # If we use AST to correct LE, make sure this is reflected
